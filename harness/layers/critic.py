    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            if ctx.saw(text):
                ...
                # Try splitting a fused contradiction claim
                ...
                for part in parts:
                    if not ctx.saw(part):
                        split_ok = False
                        break
                    # Find the doc that actually contains this part
                    doc_id = claim["doc_id"]
                    if ctx.corpus:
                        for doc in ctx.corpus.docs:
                            if doc.body in ctx.observed_text:
                                if any(part in line for line in doc.body.splitlines()):
                                    doc_id = doc.doc_id
                                    break
                    new_claims.append({"text": part, "doc_id": doc_id})
                if split_ok:
                    kept.extend(new_claims)
                    had_split = True
                # else: fabricated, drop it
        ...
        if not kept:
            ...
            report["citations"] = []
            report["abstain"] = True
            report["answer"] = "Không đủ căn cứ từ tài liệu nội bộ để trả lời câu hỏi này."
        else:
            report["claims"] = kept
            cites = []
            for c in kept:
                if c.get("doc_id") and c["doc_id"] not in cites:
                    cites.append(c["doc_id"])
            report["citations"] = sorted(cites)
